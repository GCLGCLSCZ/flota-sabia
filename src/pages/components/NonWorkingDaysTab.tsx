...
import { NonWorkingDayType, Vehicle } from "@/types";
...
const NonWorkingDaysTab = ({ vehicle }: NonWorkingDaysTabProps) => {
  ...
  const [dayType, setDayType] = useState<NonWorkingDayType>("workshop");
  ...
  useEffect(() => {
    if (vehicle && vehicle.daysNotWorked) {
      ...
      const convertedDays = vehicle.daysNotWorked.map(dateStr => {
        // Valores por defecto
        let type: NonWorkingDayType = "other";
        let details = "";
  ...
  const handleAddNonWorkingDays = async () => {
    ...
    try {
      ...
      const nonWorkingDaysDetails: Record<string, { type: NonWorkingDayType, details: string }> = {};
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Select 
                value={dayType} 
                onValueChange={(value: NonWorkingDayType) => setDayType(value)}
              >
